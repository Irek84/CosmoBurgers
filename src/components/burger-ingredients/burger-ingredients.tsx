...
import { IIngredient, IIngredientTypeList, IIngredientCard } from "../../services/types";
...
const BurgerIngredients = () => {
  ...
  const handleScroll = (e: SyntheticEvent) => {
    ...
    mainY < currentY ? setCurrentTab("mains") : sauceY < currentY ? setCurrentTab("sauces") : setCurrentTab("buns");
  };

  const buns = useMemo(() => ingredientData.filter((ingredient: IIngredient) => ingredient.type === "bun"), [ingredientData]);

  const sauces = useMemo(() => ingredientData.filter((ingredient: IIngredient) => ingredient.type === "sauce"), [ingredientData]);

  const mains = useMemo(() => ingredientData.filter((ingredient: IIngredient) => ingredient.type === "main"), [ingredientData]);

  const count = (id: string) => {
    return constructorData.ingredients?.filter((ingredient: IIngredient) => ingredient._id === id).length + (constructorData.bun?._id === id ? 2 : 0);
  };

  const IngredientsTypeList: FC<IIngredientTypeList> = ({ title, data, typeId, innerRef }) => {
    return (
      <>
  ...
  const IngredientCard: FC<IIngredientCard> = ({ data }) => {
    ...
    return (
      <li className="mt-6" ref={dragRef} style={{ opacity }}>
        <Link
          to={{
